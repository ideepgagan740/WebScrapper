...
// Delete single or Multiple Id's
router.post("/delete", async (req, res) => {
  try {
    console.log(`id's to de deleted`, req.body);

    const result = await ScrapData.deleteMany({
      _id: { $in: req.body.idsToBeDeleted }, // Use $in operator to match any of the IDs in the array
    });
...
router.post("/scrape", async (req, res) => {
  ...
  try {
    ...
    // console.log('Screenshot captured and converted to Base64',screenshotDataUrl);
  ...
  // Return the file path or URL to the screenshot
  // Assuming you are serving the screenshots from a web server,
  // you would replace this with the actual URL to access the file
  // const screenshotUrl = `http://yourdomain.com/screenshots/${path.basename(screenshotPath)}`;
